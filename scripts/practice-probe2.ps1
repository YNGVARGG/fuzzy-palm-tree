$ok=$false
$out='C:\Users\yonat\Documents\Project\practice-check2.txt'
...
try { $a = Invoke-WebRequest -Uri 'http://127.0.0.1:3001/api/tenants' -UseBasicParsing -TimeoutSec 8; Add-Content -Path $out -Value ('TENANTS ' + [int]$a.StatusCode) -Encoding utf8 } catch { Add-Content -Path $out -Value ('TENANTS ERR') -Encoding utf8 }
