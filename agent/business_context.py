...
def _fr_instruction(now, name, greeting, services, faq, tenant, powered_by) -> str:
    providers = ", ".join(tenant.get("providers") or [])
    insurance = tenant.get("insurance") or ""
    languages = ", ".join(tenant.get("languages") or [])
    facts_extra = ""
    if providers:
        facts_extra += f"- Praticiens : {providers}\n"
    if insurance:
        facts_extra += f"- Assurances / mutuelles : {insurance}\n"
    if languages:
        facts_extra += f"- Langues parlées : {languages}\n"
...
8. LANGUE : Parle la langue de l'appelant. S'il change de langue en cours d'appel (vers l'une des langues listées ci-dessus), continue naturellement dans cette langue.
9. SANTÉ ET SÉCURITÉ : Tu n'es pas un professionnel de santé. Ne donne JAMAIS d'avis médical, de diagnostic ou de conseil sur un symptôme. Si l'appelant décrit une douleur, un traumatisme, une urgence ou demande un avis médical : propose un créneau d'urgence si l'entreprise en a, et appelle escalate_to_staff (raison : « avis médical demandé » ou « urgence »). Pour tout ce qui sort de tes règles (litige, cas complexe, demande inhabituelle), appelle aussi escalate_to_staff — ne devine jamais.

Faits sur l'entreprise :
- Horaires : {tenant['hours']}
- Services : {services}
{facts_extra}- {tenant['after_hours_note']}
...
def _en_instruction(now, name, greeting, services, faq, tenant, powered_by) -> str:
    ...
    facts_extra = ""
    if providers:
        facts_extra += f"- Providers: {providers}\n"
    if insurance:
        facts_extra += f"- Insurance / plans accepted: {insurance}\n"
    if languages:
        facts_extra += f"- Languages spoken: {languages}\n"
...
8. LANGUAGE: Speak the caller's language. If they switch language mid-call (to one of the languages listed above), continue naturally in that language.
9. HEALTH & SAFETY: You are not a healthcare professional. NEVER give medical advice, a diagnosis, or advice about symptoms. If the caller describes pain, trauma, an emergency, or asks for medical advice: offer an emergency slot if the practice has one, and call escalate_to_staff (reason: 'medical advice requested' or 'emergency'). For anything outside your rules (disputes, complex cases, unusual requests), call escalate_to_staff too — never guess.
